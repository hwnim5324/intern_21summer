...
class GameManager {  // 게임 전반을 운영하는 클래스
    ...
    public void play() {
        ...
        gameOption.loadOption();  // 파일에서 저장된 옵션 데이터를 읽어 옴

        while(true){
            printMainUI();
            gamePlayer.setGame(gameOption.getLevel(),gameOption.getNumOfQuestions());
            try {
                int command = screenInput.nextInt();

                if(command == COMMAND_STARTGAME){
                    JSONArray questions = gameMaker.makeOptionalGame(gameOption.getLevel(), gameOption.getNumOfQuestions());
                    ...
                    try{
                        ...
                        else if(under_command == 2) gameOption.setLevel(1);
                        else if(under_command == 3) gameOption.setLevel(2);
                        else if(under_command == 4) gameOption.setLevel(3);
    ...
    private void printMainUI(){
        ...
        System.out.println("현재 난이도 : " + gameOption.getStrLevel() + "  현재 문항 수 : " + gameOption.getNumOfQuestions());
