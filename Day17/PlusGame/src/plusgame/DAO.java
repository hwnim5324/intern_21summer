...
public class DAO {
    ...
    public void update_OptionDB(String userId, String userLevel, String userNumofquest) throws ClassNotFoundException, SQLException {
        Class.forName(JDBC_DRIVER);
        Connection con = DriverManager.getConnection(MARIADB_URL,MARIADB_ID,MARIADB_PW);
        PreparedStatement ps;
        
        String sql_query = "UPDATE option SET userLevel=? userNumofquest=? WHERE  userId=?";
        ps = con.prepareStatement(sql_query);
        ps.setString(1, userLevel);
        ps.setString(2, userNumofquest);
